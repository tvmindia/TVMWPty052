...
namespace PartyEC.RepositoryServices.Services
{
    public class Cart_WishlistRepository: ICart_WishlistRepository
    {
                            ...
                            if ((sdr != null) && (sdr.HasRows))
                            {
                                ...
                                while (sdr.Read())
                                {
                                    ShoppingCart _ShoppingcartObj = new ShoppingCart();
                                    {
                                        ...
                                        _ShoppingcartObj.ItemID = (sdr["ItemID"].ToString() != "" ? int.Parse(sdr["ItemID"].ToString()) : _ShoppingcartObj.ItemID);
                            ...
                            if ((sdr != null) && (sdr.HasRows))
                            {
                                ...
                                while (sdr.Read())
                                {
                                    Wishlist _wishlistObj = new Wishlist();
                                    {
                                        ...
                                        _wishlistObj.StickerURL = (sdr["StickerURL"].ToString() != "" ? sdr["StickerURL"].ToString() : _wishlistObj.StickerURL);
                                    }
            ...
            try
            {
                ...
                using (SqlConnection con = _databaseFactory.GetDBConnection())
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        cmd.Connection = con;
                        cmd.CommandText = "[UpdateShoppingCartStatus]";
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@ID", SqlDbType.Int).Value = CartID;
                        statusCode = cmd.Parameters.Add("@StatusOut", SqlDbType.SmallInt);
                        statusCode.Direction = ParameterDirection.Output;
        ...
        public OperationsStatus UpdateShoppingCartQuantity(int CartID, int Qty)
        {
            OperationsStatus operationsStatusObj = null;
            try
            {
                ...
                using (SqlConnection con = _databaseFactory.GetDBConnection())
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        cmd.Connection = con;
                        cmd.CommandText = "[UpdateShoppingCartQuantity]";
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@ID", SqlDbType.Int).Value = CartID;
