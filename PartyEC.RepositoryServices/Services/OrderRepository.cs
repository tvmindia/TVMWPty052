                            ...
                            if ((sdr != null) && (sdr.HasRows))
                            {
                                ...
                                while (sdr.Read())
                                {
                                    OrderDetail orderObj = new OrderDetail();
                                    {
                                        ...
                                        orderObj.Price = (sdr["Price"].ToString() != "" ? Decimal.Parse(sdr["Price"].ToString()) : 0);
                                        orderObj.TaxAmt = (sdr["TaxAmt"].ToString() != "" ? Decimal.Parse(sdr["TaxAmt"].ToString()) : 0);
                                        orderObj.ShippingAmt = (sdr["ShippingAmt"].ToString() != "" ? Decimal.Parse(sdr["ShippingAmt"].ToString()) : 0);
                                        orderObj.DiscountAmt = (sdr["DiscountAmt"].ToString() != "" ? Decimal.Parse(sdr["DiscountAmt"].ToString()) : 0);
                                        orderObj.TotalDiscountAmt = (sdr["TotalDiscountAmt"].ToString() != "" ? Decimal.Parse(sdr["TotalDiscountAmt"].ToString()) : 0);
                                        orderObj.Total = (sdr["Total"].ToString() != "" ? Decimal.Parse(sdr["Total"].ToString()) : 0);
                                        orderObj.SubTotal = (sdr["SubTotal"].ToString() != "" ? Decimal.Parse(sdr["SubTotal"].ToString()) : 0);
                            ...
                            if ((sdr != null) && (sdr.HasRows))
                            {
                                ...
                                while (sdr.Read())
                                {
                                    OrderDetail orderObj = new OrderDetail();
                                    {
                                        ...
                                        orderObj.SubTotal = (sdr["SubTotal"].ToString() != "" ? Decimal.Parse(sdr["SubTotal"].ToString()) : 0);
                                        //orderObj.ProductQty = (sdr["ProductQty"].ToString() != "" ? int.Parse(sdr["ProductQty"].ToString()) : orderObj.ProductQty);
                                        orderObj.ShippedQty = (sdr["ShippedQty"].ToString() != "" ? int.Parse(sdr["ShippedQty"].ToString()) : orderObj.ShippedQty);
